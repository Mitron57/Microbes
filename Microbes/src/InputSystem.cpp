namespace Solution {
    ...
    std::uint32_t InputSystem::read(std::string& number) {
        ...
        while ((number[0] == '0' && number.length() > 1) ||
               std::ranges::any_of(number.begin(), number.end(),
                                   [](char elem) {
                                       return !('0' <= elem && elem <= '9');
                                   }
               )) {
            std::cerr <<
                "Number can't contain letters or begin with 0, enter number:"
                << std::endl;
            std::cout << ">> ";
            read(number);
               }
